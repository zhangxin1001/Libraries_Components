...
<script>
export default {
  props: ['numVal', 'duration', 'numLength'],
  data() {
    return {
      startVal: 0,
      interval: 100,
  ...
  methods: {
    init: function() {
      // var numString = (this.numVal ? this.numVal : 0).toString()
      var rangeLength = Math.ceil(this.duration / this.interval)
      this.duration = !this.duration ? this.defDuration : this.duration
      this.currNumlength = !this.numLength ? this.defNumLength : this.numLength
